...
RESULTS="$BASE_DIR/logs/bigger_model_matrix_${RUN_ID}.md"

mkdir -p "$BASE_DIR/logs"

cat > "$RESULTS" <<EOF
# Heimdall vLLM bigger model matrix
...
| Model | Launch | Healthcheck | 1-agent named tool call | 5-agent named tool calls | GPU memory after test | Notes |
|---|---:|---:|---:|---:|---|---|
EOF
...
run_one() {
  ...
  local model_log="$BASE_DIR/logs/bigger_model_matrix_${RUN_ID}_${safe_model}.log"
  local launch="FAIL"
  local health="FAIL"
  local one="FAIL"
  local five="FAIL"
  ...
  set_env_value "HEIMDALL_GPU_MEMORY_UTILIZATION" "0.60"
  set_env_value "HEIMDALL_ATTENTION_BACKEND" "TRITON_ATTN"
  set_env_value "VLLM_USE_DEEP_GEMM" "0"
  set_env_value "HEIMDALL_VLLM_EXTRA_ARGS" "\"--enforce-eager --enable-auto-tool-choice --tool-call-parser qwen3_xml --moe-backend triton\""
  ...
  if [[ "$health" == "PASS" ]]; then
    if python tests/test_heimdall_n_agents.py --agents 1 >>"$model_log" 2>&1; then
      one="PASS"
    else
      notes="1-agent tool-call test failed"
    fi
  fi
  ...
  gpu_mem="$(nvidia-smi --query-gpu=memory.used,memory.total,utilization.gpu,power.draw --format=csv,noheader 2>/dev/null | head -1 | tr -d '\r' || true)"
  if [[ -z "$gpu_mem" ]]; then
    gpu_mem="n/a"
  fi
  ...
  printf '| `%s` | %s | %s | %s | %s | `%s` | %s |\n' "$model" "$launch" "$health" "$one" "$five" "$gpu_mem" "$notes" >> "$RESULTS"
}
...
$(ls -1 "$BASE_DIR"/logs/bigger_model_matrix_"$RUN_ID"_*.log 2>/dev/null || true)
EOF
