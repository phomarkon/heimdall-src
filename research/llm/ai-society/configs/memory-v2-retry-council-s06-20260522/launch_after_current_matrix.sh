...
ROOT="ai-society/configs/memory-v2-retry-council-s06-20260522"
RUN_ROOT="ai-society/runs/memory-v2-retry-council-s06-20260522"
UPSTREAM_RUN_ROOT="ai-society/runs/llm-retry-council-s06-20260522"
UPSTREAM_EXPECTED=3
TARGET_SESSION="heimdall-memory-v2-retry-council-s06"

mkdir -p "$RUN_ROOT"
exec 9>"$RUN_ROOT/launch.lock"
if ! flock -n 9; then
  echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] another memory-v2 retry launcher owns the lock; exiting"
  exit 0
fi
...
wait_for_upstream() {
  while true; do
    ...
    if [ -n "$results" ] && [ -f "$results" ]; then
      ...
      case "$status" in
        failed)
          echo "[$(timestamp)] upstream retry-council matrix failed; refusing memory-v2 launch." >&2
          cat "$results" >&2
          exit 1
          ;;
        complete)
          echo "[$(timestamp)] upstream retry-council matrix complete"
          break
          ;;
      esac
    fi
    echo "[$(timestamp)] waiting for upstream retry-council matrix ($status)"
    sleep 300
  done
}

validate_configs() {
  uv run python tools/experiments/generate_memory_v2_retry_council_s06_20260522.py --check-only >/dev/null
...
run_stage() {
  ...
  mkdir -p "$log_dir"
  uv run python ai-society/run_long_model_society_matrix.py     --config-list "$list"     --log-dir "$log_dir"     --continue-on-failure     --skip-vllm-restart     > "$log_dir/controller.stdout.log" 2>&1
  local status
