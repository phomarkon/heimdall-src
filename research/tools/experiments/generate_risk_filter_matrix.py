...
ROOT = Path("ai-society/configs/risk-filter-matrix")
RUN_ROOT = Path("ai-society/runs/risk-filter-matrix")
...
ARMS = [
    ...
    {"slug": "8-risk", "agent_count": 8, "profile": "diverse_expert_action", "strategy": "comm_broadcast_digest_risk_filter", "memory": False, "max_concurrency": 6},
    {"slug": "8-risk-mem", "agent_count": 8, "profile": "diverse_expert_action", "strategy": "comm_broadcast_digest_risk_filter", "memory": True, "max_concurrency": 6},
    ...
    {"slug": "12-risk-mem", "agent_count": 12, "profile": "balanced_intelligence", "strategy": "comm_broadcast_digest_risk_filter", "memory": True, "max_concurrency": 8},
]
...
def main() -> None:
    ...
    for window_slug, start, ticks in WINDOWS:
        for arm in ARMS:
            run_id = f"rfm-{arm['slug']}-{window_slug}-f8-q32"
...
def _write_runner() -> None:
    script = RUN_ROOT / "run_risk_filter_matrix.sh"
    lines = [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "cd /home/ucloud/heimdall",
        "export PYTHONPATH='.'",
        "mkdir -p ai-society/runs/risk-filter-matrix/logs",
        "seed='ai-society/runs/memory/memory-v1-apr02.jsonl'",
        f"bank='{MEMORY_BANK}'",
        "rm -f \"$bank\"",
        "if [[ -f \"$seed\" ]]; then cp \"$seed\" \"$bank\"; fi",
        "log_dir=\"ai-society/runs/risk-filter-matrix/logs/$(date -u +%Y%m%dT%H%M%SZ)\"",
        "mkdir -p \"$log_dir\"",
        "python ai-society/run_market_intelligence_stage.py --stage risk-filter --gpu gpu0 --base-url http://127.0.0.1:8000/v1 --config-list ai-society/configs/risk-filter-matrix/gpu0.txt --log-dir \"$log_dir\" > \"$log_dir/gpu0.stdout.log\" 2>&1 &",
        "pid0=$!",
        "python ai-society/run_market_intelligence_stage.py --stage risk-filter --gpu gpu1 --base-url http://127.0.0.1:8001/v1 --config-list ai-society/configs/risk-filter-matrix/gpu1.txt --log-dir \"$log_dir\" > \"$log_dir/gpu1.stdout.log\" 2>&1 &",
        "pid1=$!",
        "echo \"$pid0 $pid1\" > \"$log_dir/pids.txt\"",
        "echo \"launched risk filter matrix log_dir=$log_dir gpu0=$pid0 gpu1=$pid1\"",
        "wait",
    ]
    script.write_text("\n".join(lines) + "\n", encoding="utf-8")
    script.chmod(0o755)
