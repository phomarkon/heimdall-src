...
ROOT = Path("ai-society/configs/regular-bid-breadth-proxy-gap-20260518")
RUN_ROOT = "ai-society/runs/regular-bid-breadth-proxy-gap-20260518"
...
def config_text(run_id: str, setup: dict[str, object], timestamp: str, ticks: int, simulator_mode: str) -> str:
...
def main() -> None:
    ...
    for window, timestamp in WINDOWS:
        for setup_name, setup in SETUPS.items():
            run_id = f"rbbpg-{setup_name}-{window}-24-regular-real-q32"
            path = ROOT / f"{run_id}.yaml"
            write(path, config_text(run_id, setup, timestamp, 24, "dual_compare_real_controls"))
            all_paths.append(str(path))

    for window, timestamp in PROXY_WINDOWS:
        for setup_name in ("s12-balanced", "s20-mixed-persona"):
            setup = SETUPS[setup_name]
            run_id = f"rbbpg-{setup_name}-{window}-24-regular-proxy-q32"
            path = ROOT / f"{run_id}.yaml"
            write(path, config_text(run_id, setup, timestamp, 24, "dual_compare_proxy_controls"))
            all_paths.append(str(path))

    for setup_name in ("s12-balanced", "s20-mixed-persona"):
        setup = SETUPS[setup_name]
        run_id = f"smoke-rbbpg-{setup_name}-apr03-1430-2-regular-real-q32"
        path = ROOT / f"{run_id}.yaml"
        write(path, config_text(run_id, setup, "2026-04-03T14:30:00Z", 2, "dual_compare_real_controls"))
    ...
    write(ROOT / "launch_after_lbfb.sh", LAUNCHER)


RUNBOOK = """# Regular-Bid Breadth + Proxy Gap Matrix
...
ROOT="ai-society/configs/regular-bid-breadth-proxy-gap-20260518"
RUN_ROOT="ai-society/runs/regular-bid-breadth-proxy-gap-20260518"
TARGET_SESSION="heimdall-rbbpg"
TARGET_STAGE="regular-bid-breadth-proxy-gap"
TARGET_EXPECTED=24
