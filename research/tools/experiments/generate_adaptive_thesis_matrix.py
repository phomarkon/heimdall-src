...
ROOT = Path("ai-society/configs/adaptive-thesis-matrix")
RUN_ROOT = Path("ai-society/runs/adaptive-thesis-matrix")
...
WINDOWS = [
    ("apr02-1200", "2026-04-02T12:00:00Z", 24),
    ("apr09-1830", "2026-04-09T18:30:00Z", 24),
]

ARMS = [
    # Baselines and best early variant.
    {"slug": "5-ind", "agent_count": 5, "profile": "diverse_action", "strategy": "diverse_action_society", "memory": False, "max_concurrency": 5},
    {"slug": "5-bcast", "agent_count": 5, "profile": "diverse_action", "strategy": "comm_broadcast_digest", "memory": False, "max_concurrency": 5},
    {"slug": "5-bcast-mem", "agent_count": 5, "profile": "diverse_action", "strategy": "comm_broadcast_digest", "memory": True, "max_concurrency": 5},
    # More diversity / more agents.
    {"slug": "8-bcast", "agent_count": 8, "profile": "diverse_expert_action", "strategy": "comm_broadcast_digest", "memory": False, "max_concurrency": 6},
    {"slug": "8-bcast-mem", "agent_count": 8, "profile": "diverse_expert_action", "strategy": "comm_broadcast_digest", "memory": True, "max_concurrency": 6},
    ...
    # Letting agents talk. Kept small-ish because peer is slow.
    {"slug": "5-peer-mem", "agent_count": 5, "profile": "diverse_action", "strategy": "comm_peer_signal", "memory": True, "max_concurrency": 3},
    {"slug": "8-peer-mem", "agent_count": 8, "profile": "diverse_expert_action", "strategy": "comm_peer_signal", "memory": True, "max_concurrency": 3},
]
...
def main() -> None:
    ...
    for window_slug, start, ticks in WINDOWS:
        for arm in ARMS:
            run_id = f"atm-{arm['slug']}-{window_slug}-f8-q32"
...
def _write_runner() -> None:
    script = RUN_ROOT / "run_adaptive_matrix.sh"
    ...
    lines = [
        ...
        "export PYTHONPATH='.'",
        "mkdir -p ai-society/runs/adaptive-thesis-matrix/logs",
        "seed='ai-society/runs/memory/memory-v1-apr02.jsonl'",
        f"bank='{MEMORY_BANK}'",
        "rm -f \"$bank\"",
        "if [[ -f \"$seed\" ]]; then cp \"$seed\" \"$bank\"; fi",
        "log_dir=\"ai-society/runs/adaptive-thesis-matrix/logs/$(date -u +%Y%m%dT%H%M%SZ)\"",
        "mkdir -p \"$log_dir\"",
        "python ai-society/run_market_intelligence_stage.py --stage adaptive-thesis --gpu gpu0 --base-url http://127.0.0.1:8000/v1 --config-list ai-society/configs/adaptive-thesis-matrix/gpu0.txt --log-dir \"$log_dir\" > \"$log_dir/gpu0.stdout.log\" 2>&1 &",
        "pid0=$!",
        "python ai-society/run_market_intelligence_stage.py --stage adaptive-thesis --gpu gpu1 --base-url http://127.0.0.1:8001/v1 --config-list ai-society/configs/adaptive-thesis-matrix/gpu1.txt --log-dir \"$log_dir\" > \"$log_dir/gpu1.stdout.log\" 2>&1 &",
        "pid1=$!",
        "echo \"$pid0 $pid1\" > \"$log_dir/pids.txt\"",
        "echo \"launched adaptive thesis matrix log_dir=$log_dir gpu0=$pid0 gpu1=$pid1\"",
        "wait",
    ]
    script.write_text("\n".join(lines) + "\n", encoding="utf-8")
    script.chmod(0o755)
