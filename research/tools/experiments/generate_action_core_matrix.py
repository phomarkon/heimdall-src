...
ROOT = Path("ai-society/configs/action-core-matrix")
RUN_ROOT = Path("ai-society/runs/action-core-matrix")
...
ARMS = [
    {"slug": "8-bcast", "agent_count": 8, "profile": "diverse_expert_action", "memory": False, "max_concurrency": 6},
    {"slug": "8-core", "agent_count": 8, "profile": "action_core_8", "memory": False, "max_concurrency": 6},
    {"slug": "8-core-mem", "agent_count": 8, "profile": "action_core_8", "memory": True, "max_concurrency": 6},
    {"slug": "12-bcast-mem", "agent_count": 12, "profile": "balanced_intelligence", "memory": True, "max_concurrency": 8},
]
...
def main() -> None:
    ...
    for window_slug, start, ticks in WINDOWS:
        for arm in ARMS:
            run_id = f"acm-{arm['slug']}-{window_slug}-f8-q32"
...
def _write_runner() -> None:
    script = RUN_ROOT / "run_action_core_matrix.sh"
    lines = [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "cd /home/ucloud/heimdall",
        "export PYTHONPATH='.'",
        "mkdir -p ai-society/runs/action-core-matrix/logs",
        "seed='ai-society/runs/memory/memory-v1-apr02.jsonl'",
        f"bank='{MEMORY_BANK}'",
        "rm -f \"$bank\"",
        "if [[ -f \"$seed\" ]]; then cp \"$seed\" \"$bank\"; fi",
        "log_dir=\"ai-society/runs/action-core-matrix/logs/$(date -u +%Y%m%dT%H%M%SZ)\"",
        "mkdir -p \"$log_dir\"",
        "python ai-society/run_market_intelligence_stage.py --stage action-core --gpu gpu0 --base-url http://127.0.0.1:8000/v1 --config-list ai-society/configs/action-core-matrix/gpu0.txt --log-dir \"$log_dir\" > \"$log_dir/gpu0.stdout.log\" 2>&1 &",
        "pid0=$!",
        "python ai-society/run_market_intelligence_stage.py --stage action-core --gpu gpu1 --base-url http://127.0.0.1:8001/v1 --config-list ai-society/configs/action-core-matrix/gpu1.txt --log-dir \"$log_dir\" > \"$log_dir/gpu1.stdout.log\" 2>&1 &",
        "pid1=$!",
        "echo \"$pid0 $pid1\" > \"$log_dir/pids.txt\"",
        "echo \"launched action core matrix log_dir=$log_dir gpu0=$pid0 gpu1=$pid1\"",
        "wait",
    ]
    script.write_text("\n".join(lines) + "\n", encoding="utf-8")
    script.chmod(0o755)
