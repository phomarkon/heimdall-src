...
MATRIX = "llm-retry-council-s06-20260522"
...
def main() -> None:
    ...
    for window_slug, timestamp in WINDOWS.items():
        run_id = f"lrc-s06-actioncore-retry-{window_slug}-seed{SEED}-24-q32"
        ...
        smoke_id = f"smoke-lrc-s06-actioncore-retry-{window_slug}-seed{SEED}-2-q32"
...
def _write_manifest(full_configs: list[Path], smoke_configs: list[Path]) -> None:
    payload = {
        ...
        "question": "Can peer summaries plus retry feedback make guarded Qwen3-32B convert more exact simulator-accepted candidates without increasing unsupported bids?",
        ...
        "memory_enabled": False,
        "windows": WINDOWS,
        "chain_after": {"matrix": "det-llm-critic-20260522", "required_completed": 3, "required_failed": 0},
...
def _row(path: Path) -> dict[str, Any]:
    ...
    return {"run_id": payload["run_id"], "config": str(path), "start_timestamp": payload["start_timestamp"], "ticks": payload["ticks"]}
...
def _sanity_check(full_configs: list[Path], smoke_configs: list[Path]) -> None:
    ...
    if len({payload["run_id"] for payload in payloads}) != len(payloads):
    ...
    if {_load_config(path)["ticks"] for path in full_configs} != {24}:
        raise RuntimeError("full configs must be 24 ticks")
    if {_load_config(path)["ticks"] for path in smoke_configs} != {2}:
...
def _assert_no_existing_outputs(run_ids: set[str]) -> None:
    ...
    if collisions:
        raise RuntimeError("refusing duplicate completed llm-retry-council outputs:\n" + "\n".join(collisions))


RUNBOOK = """# LLM Retry Council S06 2026-05-22

S06-only retry-council run using the current scenario-envelope settings. This is the no-memory pair for the memory-v2 ablation.
"""
...
ROOT="ai-society/configs/llm-retry-council-s06-20260522"
RUN_ROOT="ai-society/runs/llm-retry-council-s06-20260522"
UPSTREAM_RUN_ROOT="ai-society/runs/det-llm-critic-20260522"
UPSTREAM_EXPECTED=3
TARGET_SESSION="heimdall-llm-retry-council-s06"

mkdir -p "$RUN_ROOT"
exec 9>"$RUN_ROOT/launch.lock"
if ! flock -n 9; then
  echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] another retry-council launcher owns the lock; exiting"
  exit 0
fi
...
latest_upstream_results() {
  find "$UPSTREAM_RUN_ROOT"/logs-[0-9]* -name results.json -type f -printf '%T@ %p\\n' 2>/dev/null | sort -n | tail -1 | cut -d' ' -f2-
}
...
wait_for_upstream() {
  while true; do
    ...
    if [ -n "$results" ] && [ -f "$results" ]; then
      ...
      case "$status" in
        failed)
          echo "[$(timestamp)] upstream critic matrix failed; refusing retry-council launch." >&2
          cat "$results" >&2
          exit 1
          ;;
        complete)
          echo "[$(timestamp)] upstream critic matrix complete"
          break
          ;;
      esac
    fi
    echo "[$(timestamp)] waiting for upstream critic matrix ($status)"
    sleep 300
  done
}

validate_configs() {
  uv run python tools/experiments/generate_llm_retry_council_s06_20260522.py --check-only >/dev/null
...
run_stage() {
  ...
  local expected="$2"
  local prefix="$3"
  local log_dir="$RUN_ROOT/$prefix-$(date -u +%Y%m%dT%H%M%SZ)"
  mkdir -p "$log_dir"
  ...
  local status
  status="$(stage_status "$log_dir/results.json" "$expected")"
  if [ "$status" != "complete" ]; then
    echo "[$(timestamp)] $prefix failed or incomplete: $status" >&2
...
uv run python ai-society/run_long_model_society_matrix.py \
  ...
  > '$log_dir/controller.stdout.log' 2>&1
"
  echo "[$(timestamp)] launched $TARGET_SESSION log_dir=$log_dir"
}

wait_for_upstream
validate_configs
run_stage "$ROOT/smoke.txt" 3 "logs-smoke"
launch_full_stage
"""
